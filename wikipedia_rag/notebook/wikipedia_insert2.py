...
# 로깅 설정
log_filename = f"wikipedia_insert2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
...
# df를 뒤에서부터 처리
for batch_start in tqdm(range(len(df) - 1, -1, -batch_size), desc="Processing batches"):
    batch_end = max(batch_start - batch_size + 1, 0)
    batch_df = df.iloc[batch_end:batch_start + 1]
    documents = []
    
    try:
        logger.info(f"Starting batch processing from index {batch_end} to {batch_start}")
        ...
        logger.info(f"Successfully completed batch {batch_end} to {batch_start}")
        
    except Exception as e:
        logger.error(f"Error occurred at batch starting with index {batch_end}: {str(e)}")
